...
{
    clearVisited(graph);
    if (source == destination) //checks if they are the same, then no search is needed.
    {
        return 1;
    }
    Vertex* curVert; //the current vertex being searched
    Deque* dPaths = dequeNew(); //make a new deque
    dequePushBack(dPaths, source); //push the source to the back

    while (dequeIsEmpty(dPaths) != 1) //while the deque is not empty; while there are still vertices to search
    {
        curVert = dequeBack(dPaths); //curVert is the vertex at the back of the deque, the "deepest" vertex
        dequePopBack(dPaths); //pop it off the back because it has been visited
        if (destination == curVert) //if the current vertex is the destination, then stop and return found
        {
            dequeDelete(dPaths);
            return 1;
        }
        curVert->isVisited = 1; //set visited to 1
        int i;
        for (i = 0; i < curVert->numNeighbors; i++) //get the neighbors
        {
            if (curVert->neighbors[i]->isVisited == 0) //get only the unvisited neighbors
            {
                dequePushBack(dPaths, curVert->neighbors[i]); //add them to the back, so the newest neighbors added always get searched first
            }
        }
    }
    dequeDelete(dPaths); //if not found, delete the deque and return not found
    return 0;
}
/**
 * Determines if there is a path from the source to the destination using an
...
//This function is almost exactly the same as the one above, except for on part, which is indicated in the comments.
int bfsIterative(Graph* graph, Vertex* source, Vertex* destination)
{
    ...
    Deque* bPaths = dequeNew();
    Vertex* curVert;
    dequePushBack(bPaths, source);
    while (dequeIsEmpty(bPaths) != 1)
    {
        //here is the key difference. getting the vertex at the front means getting the neighbor to the "parent" of the one just checked
        //Getting from the back means getting an actual neighbor to the one just checked.
        curVert = dequeFront(bPaths);
        dequePopFront(bPaths);
        if (destination == curVert)
        {
            dequeDelete(bPaths);
            return 1;
        }
        ...
        for (i = 0; i < curVert->numNeighbors; i++)
        {
            ...
            {
                dequePushBack(bPaths, curVert->neighbors[i]); //push to back because we don't want to go too deep, we want to evaluate the closer neighbors first.
            }
        }
    }
    dequeDelete(bPaths);
